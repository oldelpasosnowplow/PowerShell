#Note you will need the WSUS tools from RSAT setup on your machine to access
#ProgramFiles\Update Services\Api\Microsoft.UpdateServices.Administration.dll 

#######################################################################################
#Fill in the top part to match your network settings                                  #
#######################################################################################

Param (
[string]$UpdateServer = 'WSUS Servername goes here', # This is the WSUS server name.
...
$smtp = "smtp.doamin.local" # This is your SMTP Server
$smtpPort = 25
$enableSSL = $False
$htmlBody = $true
$to1 = "ITNOTIFICATION@domain.com" # This is the recipient smtp address 1
...
# UpdateServer + "<" + $UpdateServer + "@domain.com>" 
# This will be the sender´s address identifying the WSUS server
...
$MsgBody = $MsgBody + "<title>Computer report on " + $UpdateServer + "</title>"
$MsgBody = $MsgBody + "</HEAD>"
$MsgBody = $MsgBody + "<BODY style=""font-family:'Courier New', Courier, monospace"">"
$MsgBody= $MsgBody + "<h1>Computer report on " + $UpdateServer + "</h1>"
...
$MsgBody = $MsgBody + "<table border=""1"" cellspacing=""1"" cellpadding=""1"" style=""font-family:'Courier New', Courier, monospace"">"
$MsgBody = $MsgBody + "<tr>"
...
# Now send the email message and thats all.
    ...
    $mailer = New-Object System.Net.Mail.SmtpClient ($smtp, $smtpPort)
    $mailer.EnableSSL = $enableSSL
    if ($AdamjMailReportSMTPServerUsername -ne "") {
        $mailer.Credentials = New-Object System.Net.NetworkCredential('', '')
    }
    $message.From = $from
    $message.To.Add($to1)
